...
public:
  ...
  VendingMachine(double coin_denominations[6], double quantity_of_coins[6], string cars[5], double quantity[5], double price[5], double need_to_refill[5])
      : Product(coin_denominations, quantity_of_coins, cars, quantity, price, need_to_refill)
  { // constructor
  }
  ...
  void returning_money()
  {
    ...
    while (rest_of_money > 0)
    {
      ...
      for (int i = 0; i < 6; ++i)
      {
        std::cout << "Denomination: " << coin_info[i][0] << ", Quantity: " << coin_info[i][1] << std::endl;
      }

      for (int i = 0; i < 6; i++)
      {

        if (double(rest_of_money) >= double(coin_info[i][0]))
        {
          for (int j = 0; j < 6; j++)
          {
            if (coin_info[i][0] == denomination_of_coins[j])
            {
              while (coin_info[i][1] >= coin_info[i + 1][1] && rest_of_money >= coin_info[i][0])
              {
                ...
                cout << "picked coin denomination: " << coin_info[i][0] << "zł"
                     << "\n";
                cout << "coresponding coin quantity: " << coin_info[i][1] << "\n";
        ...
        {
          cout << "cannot pick coin denomination "
               << "\n";
        }
